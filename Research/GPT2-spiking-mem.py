# coding=utf-8
# Modified based on OpenAI GPT-2 model by Yinghan Long
"""PyTorch GPT-2 with spiking memory."""
...
class GPT2MLP-Spiking(nn.Module):
    def __init__(self, intermediate_size, config):
        super().__init__()
        embed_dim = config.hidden_size
        self.c_fc = Conv1D(intermediate_size, embed_dim)
...
class SpikingMem(nn.Module):
    def __init__(self, seq_len, batch, config):
        super().__init__()
        embed_dim = config.hidden_size
        self.fc = Conv1D(seq_len, seq_len)
        self.mem = torch.zeros(batch, seq_len, embed_dim)
        self.act = ACT2FN[config.activation_function] #TODO: use activation function for spiking NN
        #TODO:add a mask for uni-direction 

    def forward(self, h):
        h = h.transpose(1,2)
        h = self.fc(h)
        self.mem = self.mem + h
        out= self.act(self.mem)
        return out
